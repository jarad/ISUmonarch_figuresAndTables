...
```{r daubenmire_yearly_plot_by_class, dependson=c("daubenmire_yearly","palette"), fig.height=8, fig.cap="Trends in yearly cumulative percent cover by vegetative class and bare soil from 2016 through 2019 (based on results for Daubenmire surveys; see Table 2 and Figure 3). "}
...
ss <- d %>%
  ...
  summarize(total = sum(count)) %>%
  ungroup %>%
  tidyr::complete(nesting(common_name, native), year, site, `Bee Species`, fill = list(total=NA)) %>%
  mutate(native = factor(native, 
                         levels=c("Native (planted)",
                                  "Native (extant)",
                                  "Introduced/weed"))) %>%
  
  # cra1 has a 70m transect, so adjust to 100m total
  mutate(total = ifelse(site == "cra1", total/.7, total),
         `Bee Species` = gsub(" bee","",`Bee Species`)) %>%
  unite("site_year", site, year, sep="@") %>%
  spread(site_year, total, fill = NA) %>%
  arrange(`Bee Species`, native, common_name) %>%
