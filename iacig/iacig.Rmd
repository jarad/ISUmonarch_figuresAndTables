...
```{r daubenmire_yearly_plot_by_class, dependson=c("daubenmire_yearly","palette"), fig.height=8, fig.cap="Need a caption"}
...
```{r nectar_yearly_plot, dependson="nectar_yearly_data",fig.cap="Plots of monthly average native forbs, extant native forbs, introduced/weed species, and combined (native and introduced/weed) nectar plant inflorescence density by year for each site."}
g <- ggplot(s_both, aes(year, total, 
              linetype = month, group = month, color = month)) +
  geom_line() + 
  facet_grid(native ~ site, scales="free_y") + 
  labs(x = "Year", y = "Average Inflorescence (count/m2)", 
       title = "Nectar Plant Survey",
...
tab <- ss %>%
  xtable(
    digits = 0,
    align = paste("rl|",paste(rep("rrrr|",floor(nc/4)),collapse=""),sep="",collapse=""),
    caption="Average inflourescence (count/100m2) across all rounds. Species above the double line are native (planted or extant) and species above single line were planted.",
    label = "t:nectar") %>%
  print(
...
g <- ggplot(d %>%
  ...
  labs(x = "Year", y = "Count",
       title = "Yearly Monarch Adults, Eggs, Larvae, and Ramets") +
  # scale_y_log10() + 
  theme_bw() +
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle=90))
...
```{r bee_plant_table, dependson="bee_plants_data",eval=FALSE}
ss <- d %>%
  group_by(year, site, common_name, native) %>%
  summarize(density = mean(total)) %>%
  mutate(native = factor(native, levels=c("Native (planted)","Native (extant)",
