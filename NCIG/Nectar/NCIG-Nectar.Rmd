...
nectar_density_table <- function(yr, flt, label) {
  ...
  left_join(subset(ISUmonarch::survey, year==yr) %>% select(transectID, length, round), by=c("transectID", "round")) %>%
  
  group_by(`Nectar Plant Species`, transectID, round) %>%
  summarize(density = 100*sum(count, na.rm=TRUE)/as.numeric(unique(length))) %>%
